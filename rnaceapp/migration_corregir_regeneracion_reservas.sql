-- Migración para corregir regenerar_reservas_futuras
-- Corrige el bug donde no se filtraba por modalidad, causando overbooking incorrecto.
-- También asegura que se limpien reservas obsoletas de asignaciones automáticas anteriores.

-- Primero eliminamos la función anterior para evitar errores de tipo de retorno
DROP FUNCTION IF EXISTS regenerar_reservas_futuras();

CREATE OR REPLACE FUNCTION regenerar_reservas_futuras()
RETURNS json
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    ...
    LOOP
        -- 2. Iterar sobre todos los usuarios activos tipo cliente
        FOR v_usuario IN 
            SELECT id FROM usuarios WHERE rol = 'cliente' AND activo = true
        LOOP
            ...
            LOOP
                ...
                IF v_key = ANY(v_horarios_usuario) THEN
                    ...
                    IF NOT FOUND THEN
                        -- Crear reserva
                        INSERT INTO reservas (usuario_id, sesion_id, estado, es_desde_horario_fijo, created_at)
